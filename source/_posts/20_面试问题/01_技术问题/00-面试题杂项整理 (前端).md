...
5. 重绘(Repaint) 和 回流(Reflow)
	重绘: 节点需要更改 外观(不影响布局)，如改变color就是重绘
	回流: 布局或者几何属性(影响布局) 改变就是回流
	减少重绘和回流：
	  1)批量修改 DOM
	  2)复杂动画效果，使用绝对定位使其脱离文档流
...
2. 如何最小化 重绘(repaint) 和 回流(reflow)？
    1)对元素复杂操作时，先隐藏（display:none），操作完再显示
    2)创建多个DOM节点时，使用 DocumentFragment 创建完后一次性的加入 document
...
5. 前后端对接优化课程播放页高并发场景的优化
    ...
    前端: 1)接口缓存:localStorage-有效期5分钟-重复进入仅首次请求接口  2)防抖节流:点赞/收藏都做了(500ms内单用户仅触发1次请求)lodash.debounce封装
