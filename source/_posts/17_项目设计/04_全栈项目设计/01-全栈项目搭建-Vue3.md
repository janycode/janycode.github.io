...
>
> 功能：**基于蚂蚁地理空间数据可视化的地图标注系统、包含一整套用户管理、菜单权限、登录校验、预约审核等功能，项目搭建快速起步架子**。
>
> 一页效果：
>
...
[UserList.vue](https://github.com/janycode/lab-system-admin-web/blob/main/src/views/user-manage/UserList.vue) - 增删改查+列表分页 完整逻辑。
...
效果：每个页面进入时，顶部从左向右一条很细的加载进度条。



### 3.6 前后端分页整合

以 UserList.vue 用户列表为例，效果：

![image-20260116110902351](https://jy-imgs.oss-cn-beijing.aliyuncs.com/img/20260116110903744.png)

#### 前端列表

[UserList.vue](https://github.com/janycode/lab-system-admin-web/blob/main/src/views/user-manage/UserList.vue) - ①新增分页组件 ②分页配置和列表分页请求
...
const getList = async () => {
  ...
  try {
    ...
    pagination.total = res.data.total  //分页必须赋值总条数
  } catch (err) {
...
![image-20260116111329801](https://jy-imgs.oss-cn-beijing.aliyuncs.com/img/20260116111331058.png)
