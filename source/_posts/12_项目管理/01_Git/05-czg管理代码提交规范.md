...
# 使用 czg 管理代码提交：日常开发完整流程（含配置、提交、定制化）
...
# 完整配置 Husky 强制提交规范（适配 czg + Conventional Commits）
...
#### 步骤 4：配置 commitlint 校验规则（强制提交信息格式）
...
#### 步骤 5：添加 Husky 钩子（核心：强制校验）

添加 2 个关键钩子：`pre-commit`（提交前校验）、`commit-msg`（校验提交信息格式）。

##### ① 添加 commit-msg 钩子（强制提交信息符合规范）
...
##### ② 添加 pre-commit 钩子（强制使用 czg 提交，禁止直接 git commit）
...
#### 步骤 6：配置 package.json 快捷脚本（可选，提升团队使用效率）
...
#### 测试 1：直接使用 git commit 提交不规范信息（应失败）
