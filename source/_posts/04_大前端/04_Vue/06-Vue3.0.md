...
缺点：无法监听数组的改变，无法监听class改变，无法监听Map,Set结构。
...
（2）双大括号不能在HTML attributes中使用。想要响应式地绑定一个attribute，应该使用v-bind 指令 template
...
XMLHttpRequest API, XHR的配置和调用方式混乱，而且基于事件的异步模型写起来不友好。  
