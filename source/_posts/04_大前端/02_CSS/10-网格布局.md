...
绝对大小（根据列数或行数确定值的个数），如 200px 200px 200px
...
根据列数或行数确定值的个数，如 33.33% 33.33% 33.33%
...
功能函数：`repeat(次数, 值)` - 值可以是 百分比 / 像素 / fr列宽片段

```css
/*  参数1：重复的次数
    参数2：重复的数值或者重复的模式，百分比 / 像素 / fr列宽片段
*/
eg:
