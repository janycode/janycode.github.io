...
* `E[attr~="value"]`，指定属性名，并且有属性值，此属性值是一个词列表，并且以空格隔开，其中此列表中包含了一个value词，而且等号前面的 "~" 必须写
...
* `:root`，匹配文档的根元素，在HTML（标准通用标记语言下的一个应用）中，`根元素永远是 <html>`
...
* `E:not(s)`，匹配所有不是简单选择符s的元素E（IE6-8浏览器不支持）
