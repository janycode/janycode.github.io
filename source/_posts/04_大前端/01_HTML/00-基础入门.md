...
* **auto rename tag**：自动`成对修改标签插件`，默认生效，不需要额外操作
...
* **Comment Translate**： 自动`翻译` MDN reference 提示信息，最好设置一下源语种和目标语种
* **px to rem & rpx & vw (cssrem)**：自动计算rem值，快捷键 `Alt+Z` 用于切换 rem与px
