...
11.高并发，处理时间短，使用哪种线程池？低并发，处理时间长，使用哪种线程池？高并发，处理时间长，如何处理？
    ...
    低并发处理时间长：根据业务场景使用7个参数的ThreadPoolExecutor，区分CPU密集型与IO密集型设置参数（参考17.）
...
17. CPU密集型和IO密集型 线程参数设计
    1)CPU 密集型（如计算、排序、加密）
    瓶颈：CPU 算力，线程数过多导致上下文切换频繁，反而降速；
    参数：
    - corePoolSize = maximumPoolSize = CPU核心数 ± 1（如 8 核 CPU 设 8 或 9）；
    - 阻塞队列用LinkedBlockingQueue（无界 / 固定容量，减少线程创建销毁）；
    - 拒绝策略：CallerRunsPolicy（核心线程忙时让调用线程执行，避免任务丢失）。
    2)IO 密集型（如 DB 查询、Redis 交互、接口调用）
    瓶颈：IO 等待（线程大部分时间闲置），需更多线程利用空闲时间；
    参数：
    - corePoolSize = CPU核心数*2 或 CPU核心数/(1-阻塞系数)（阻塞系数≈0.9，8核≈80）；
    - maximumPoolSize = corePoolSize（或略大，如 + 10）；
    - 阻塞队列用ArrayBlockingQueue（有界，避免任务堆积 OOM）；
    - 拒绝策略：DiscardOldestPolicy（丢弃最旧任务，保证新任务执行）。
```
...
```java
1. 堆内存划分？高并发项目中OOM问题怎么定位的？
    堆内存划分：新生代、老年代、永久代
    Eden区 和 Survivor区比例： 8:1:1 (两个Survivor各站10%)
    实战中遇到过OOM的情况 OutOfMemoryError（场景: 用户在高峰期会大量涌入活动页），主要是分析 dump文件(通过jmap -dump命令生成dump)。
        使用jdk自带的 VisualVM 或 MAT分析，查看对象内存占用情况，定位大量创建的如List对象未被及时回收。
    解决: 1)优化业务逻辑  2)减少对象创建  3)调整JVM堆参数（根据内存情况适当增大）

2. MySQL索引类型有哪些？聚簇索引和非聚簇索引的区别？慢查询优化实际案例？
    主键索引、普通索引、唯一索引、全文索引、组合索引（最左匹配原则）
    聚簇索引: 即主键索引，非叶子节点存储索引、叶子节点存储行数据，每张表只有一个聚簇索引。
    非聚簇索引: 存储了当前行数据的'主键值'，所以每次查询时需要二次查询主键索引树，也就产生了回表。
    慢查询优化案例: 学员课程学习表查询操作有过滤和排序等条件，表数据量约500w多，查询条件中有字段没有索引，所以触发了全表扫描。
        解决: 将条件查询字段按照最左匹配原则建立联合索引，查询时间从2s多优化到了60ms内
            
3. JVM 参数层面你做了哪些具体调优？有没有开启 G1 垃圾收集器？调优后线上 OOM 的发生频率有什么变化？
    JVM参数调优: -Xmx增大堆内存，-Xmn增大新生代空间，如 -Xmx4g -Xmn1g 让新生代容纳更多短期存活对象
    G1垃圾收集器: 需要开启，-XX:+UseG1GC 利用其并行和并发收集的优势
    大对象参数: -XX:PretenureSizeThreshold 设置为 10M，使大于此阈值的大对象直接进入老年代，避免新生代频繁复制。
    OOM频率变化: 从之前的每天2次，降低到了0次，基本活动推广时基本不再出现了
        
4. 索引失效场景？如何排查解决？mysql读写分离？主从同步延迟问题如何解决？
   索引失效: 隐式类型转换（比如主键id是int类型，但是给了个字符串类型的值就会失效），以及 like % 或 联合非索引字段查询等
   排查解决: 使用 explain 查询 key 字段，判断是否使用索引
   解决办法: 类型匹配、like xxx%前缀匹配、or 字段都添加索引等
   mysql读写分离: mybatisPlus 提供了读写分离的支持，使用动态数据源绑定主库和从库，使用AOP切面实现读写分离，主库用于写，从库用于读
   mysql主从同步延迟问题: 1)减少主库的写入压力 2)提升从库的性能 3)配置参数降低主库I/O，从库使用多线程 4)监控和预警及时处理
       
5. 你的微服务是按什么维度拆分的？拆分后遇到过 “服务调用超时/熔断” 的问题吗？流量控制怎么做？列举案例
   微服务拆分: 按照业务模块拆分，比如拼团、秒杀、抽奖、砍价等独立业务模块
   拆分后问题: 确实出现了超时和熔断问题，主要在秒杀业务上
   流量控制: 在优化了代码和sql后，同时使用 sentinel 做限流和降级处理，配置了限流规则、降级规则、系统自适应保护
   活动高峰期接口 QPS 从 1 万突增到 5 万，通过 Sentinel 单机限流 800 QPS，集群总限流 3000 QPS（4台服务器）

6. 分布式缓存 reids场景？穿透、击穿、雪崩？秒杀超卖问题？
   主要使用场景是: 分布式锁
   分布式锁: 在秒杀业务中使用，秒杀的核心接口，会对 活动id+用户id+商品id 设置分布式锁，单用户对单商品的精准控制（如限购1件）
   - 穿透: 不存在的key遭到大量请求 - 布隆过滤器+空值缓存
   - 击穿: key过期后大量请求进入DB - 互斥锁+热点key永不过期
   - 雪崩: 大量key同时过期redis宕机 - 过期时间随机化+多级缓存处理
   秒杀超卖问题：
       Redis incr/decr原子操作预减库存抗流量 + MQ削峰 + 乐观锁(库存>0) + 分布式锁(控制最后1件不超卖) + DB事务校验。

7. 后端是如何给前端做 “接口兜底” 的？后端数据库挂了如何降级？前端参数如何统一校验？
    本地缓存Caffeine -> redis缓存 -> DB数据库
    后端数据库挂了，返回 redis 缓存数据 + 前端友好提示。
    统一参数校验拦截使用 Validation + 全局异常处理捕获；也可以通过自定义注解 + AOP切面来实现业务逻辑定制化的校验规则

8. 接口高可用设计做过哪些？
   幂等控制重复请求 + Sentinel 限流熔断降级 + 多级缓存弱依赖 DB
     幂等性: 前端传入 课程ID+用户ID+播放记录ID 作为唯一标识，接口中通过redis设置为分布式锁，同一用户同一课程短时间内只处理一次，入库前校验避免重复记录
     Sentinel: 单机/集群QPS限流-超过返回友好提示、下游依赖服务配置熔断、熔断后降级处理返回缓存数据（本地缓存或redis），确保业务流程不出错
     多级缓存设计: 本地缓存Caffeine -> redis 缓存 -> DB(只做异步落库，核心流程不依赖DB)

9. 全栈视角下前后端联调提效？如何看待全栈开发与前后端分离？
    - 统一前后端'响应体': 后端 Result<T> 前端规范处理
    - '接口文档'自动化: 使用swagger + ApiFox（联调/自测均可高效落地）
    - 'Mock'提前并行联调: 根据约定字段前后端均可提前联调
    - 统一请求/响应'拦截': 前端axios统一拦截、统一处理token、超时、错误码、限流熔断提示；后端全局异常处理器拦截
    - 接口'约定前置': 先定入参、出餐、URL/api规范，再开发，避免返工
    前后端分离是架构标准: 大型项目协作、职责清晰、专业深耕、可维护性等，适合多人、复杂业务
    全栈开发是能力升级: 单人跑通全链路，更易规避协作带来的沟通成本、时间成本，从需求-接口-缓存-数据库-限流-页面交互 都能闭环

10. 分布式事务：
    如果后续遇到课程购买的分布式事务场景，会优先选择 'Seata AT' 模式（因为对业务入侵小，适合中小微服务场景）
    或基于 MQ 实现最终一致性（适合对实时一致性要求不高的 SaaS 场景）

11. AI 是全栈开发者的超级放大器，它解放了编码，却更凸显架构、业务与系统设计的价值。
    在我看来，AI 对全栈开发是效率放大器，而不是替代者。
    它把重复编码、接口调试、文档生成这些体力活大幅简化，让我能更专注架构设计、高可用、并发安全、业务逻辑这些核心价值。
    当下的全栈，不再是单纯写代码，而是用 AI 工具快速落地完整系统，一人就能完成从需求到上线的全流程。
    我始终认为：代码可以交给 AI，但方案、判断、质量把控，必须由开发者自己掌握。
```
...
1.Nacos注册中心实现原理：服务注册 + 服务发现
    ...
    5)注册中心 检测到服务提供者异常，则通过'UDP'协议推送更新
    调用方发起调用时，通过注册中心的服务实例名称，找到ip地址端口发送请求和接收响应
    PS：nacos依赖包中包含 ribbon, 因此默认负载均衡为 '轮询' 机制。
2.Nacos 是 AP 还是 CP？
  默认'AP'（高可用），网络分区时依然能读写、保证服务可用；可切换为CP（强一致），用于配置、元数据等强一致场景。
  //做注册中心用 AP，做配置中心用 CP
3.Nacos 配置动态刷新原理？
  客户端长轮询Nacos（30s 超时）；配置变更时 Nacos 立即返回，客户端拉取最新配置；通过 @RefreshScope 实现 Bean 热更新，不用重启。
4.Nacos 修改配置不生效如何排查解决？
  ①客户端是否加@RefreshScope或配置监听；
  ②配置 DataID/Group/命名空间是否和客户端匹配；
  ③客户端是否拉取到最新配置（日志/接口验证）。
  解决：补注解、校准配置标识、重启客户端/刷新 Nacos 连接，最终通过 Nacos 接口确认配置推送状态。
...
1.ZK 的节点类型有哪些？
  ...
  //服务注册常用临时节点，掉线自动剔除。
2.ZK 是 AP 还是 CP？
  强一致性 'CP'。半数以上节点写入成功才算成功；少数节点故障不影响，但网络分区时不可写，保证数据一致。
3.ZK 分布式锁原理？
  创建临时有序节点，客户端判断自己是否'最小节点'；是则获取锁，否则监听前一个节点。
  锁释放 / 断开会话，临时节点删除，唤醒下一个。

Apollo
1.Apollo 配置实时推送原理？
  服务端用'长连接推送'，客户端实时接收；配合'定时拉取'兜底，保证配置不丢失；比轮询更实时、性能更高。
2.Apollo 灰度发布是什么？
  配置先'推给部分实例'，验证没问题再全量发布；支持按 IP、集群灰度，一键回滚，避免故障扩散。
3.Apollo 和 Nacos 配置区别？
  Apollo：企业级、权限、灰度、审批、版本管理更强；
  Nacos：轻量、简单、和微服务生态更贴合；
  //中小型用 Nacos，大型企业/强管控用 Apollo。
```
...
```java
RocketMQ核心: “事务 / 延迟消息”，场景：秒杀流量削峰
RabbitMQ核心: “交换机 / 死信”，场景：通知推送广播
Kafka核心: “分区 / 零拷贝 / 重平衡”，场景：日志采集
...
3.RocketMQ怎么解决消息丢失和重复消费、顺序消费问题:
	消息丢失：即消息可靠性
        ①发送可靠性-生产端：同步-直到服务器返回成功,默认重试2次 / 异步-开新线程回调接口 / 单向-不可靠,不建议
        ②存储可靠性-服务端：底层同步/异步刷盘逻辑保证
        ③消费可靠性-消费端：消费重试（时间level递进重试）、死信队列（相关接口单独获取）、消息回溯（按时间回退再次消费）
	重复消费：消费方'幂等'性处理，数据库/redis的唯一业务id实现
        ①基于唯一键（订单号 / 消息 ID） + Redis SETNX校验
        ②数据库唯一索引兜底
        ③消费逻辑设计为可重复执行（无副作用）
...
4.RocketMQ 与 Kafka 核心区别？
    ①场景：RocketMQ 适配金融 / 电商（事务消息、延迟消息），Kafka 侧重大数据日志；
    ②架构：RocketMQ 主从架构，Kafka 分区副本；
    ③功能：RocketMQ 支持事务 / 延迟 / 死信，Kafka 无原生支持。

RabbitMQ
1.RabbitMQ 交换机类型及使用场景？
    ①直连交换机（Direct）：精准路由（按 RoutingKey 匹配），用于订单/支付；
    ②扇形交换机（Fanout）：广播消息，用于日志/通知；
    ③主题交换机（Topic）：模糊匹配（通配符），用于多维度路由。
2.RabbitMQ 如何处理死信（延时）消息？
    ①死信触发：消息过期 / 队列满 / 消费拒签且不重回队列；
    ②配置死信交换机 + 死信队列，接收死信消息；
    ③人工排查死信原因，重发或补偿处理。
3.RabbitMQ 消息堆积怎么解决？
    ①临时扩容消费者节点，提高消费能力；
...
1. Kafka 分区和副本的作用？
    ① 分区：水平扩展，提高并发读写，消息按 Key 哈希分区；
    ② 副本：保障高可用，leader 负责读写，follower 同步数据；
    ③ 副本数 > 1，leader 故障时 follower 自动切换。
2. Kafka 消费者重平衡（Rebalance）原因及避免？
    ① 原因：消费者上下线、分区数变更、订阅主题变更；
    ② 避免：设置合理session.timeout.ms，用静态分配分区，减少重平衡触发；
    ③ 影响：重平衡期间消费暂停，需控制频率。
3. Kafka 高吞吐的核心原因？
    ① 磁盘顺序写（比随机写快 10 倍）；
    ② 零拷贝（数据直接从内核到网卡，减少拷贝）；
    ③ 批量读写 + 压缩传输，降低 IO / 网络开销；
    ④ 分区并行处理，充分利用多核。
```
...
```java
Feign「动态代理、注解映射、熔断降级」，主打轻量、和 SpringCloud 生态融合；声明式 HTTP RPC，基于 Restful，适配微服务轻量场景。
Dubbo「架构流程、负载均衡、超时重试」，主打高性能、丰富的治理能力；原生 RPC，基于 TCP，性能更高，治理能力（负载均衡 / 重试）更完善。
...
2.常见RPC框架和区别：OpenFeign、Dubbo、gRPC、Motan(微博)、Tars(腾讯)
3.OpenFeign实现原理：
...
4. Feign 的核心工作原理？
    ① 基于接口注解生成'动态代理'类；
    ② 代理类封装请求（URL / 参数 / 请求方式）；
    ③ 交给'Ribbon'做负载均衡，调用远程服务；
    ④ 解析响应结果，封装为接口返回值。
5. Feign 如何实现请求参数传递 / 自定义拦截？
    ① 参数传递：用@RequestParam/@PathVariable/@RequestBody注解映射；
    ② 自定义拦截：实现RequestInterceptor，重写apply方法，添加请求头（token/签名）；
    ③ 配置类注入拦截器，全局生效。
6. Feign 结合 Sentinel 做熔断降级的实现？
    ① 引入spring-cloud-starter-alibaba-sentinel依赖；
    ② 配置feign.sentinel.enabled=true开启熔断；
    ③ 给 Feign 接口指定降级类（fallback），实现失败兜底逻辑。
...
2.Dubbo集群容错方式、代理方式：
    Dubbo 默认容错是 Failover（失败重试），核心 6 种：
        ①Failover 失败重试（默认，幂等接口用）；
        ②Failfast 快速失败（非幂等 / 写操作）；
        ③Failsafe 失败安全（日志 / 监控）；
        ④Failback 失败自动恢复（通知 / 消息）；
        ⑤Forking 并行调用（实时性要求高）；
        ⑥Broadcast 广播调用（更新配置），按业务场景选对应策略。
        //秒杀场景用 Forking 容错，订单创建用 Failfast，查询接口用默认 Failover
    Dubbo 有 2 种代理方式：
        ①JDK 动态代理（默认，基于接口），适配有接口的场景；
        ②CGLIB 动态代理（基于类），适配无接口的场景；核心是生成代理类封装远程调用逻辑，通过 SPI 机制切换代理方式。
3.Dubbo的spi机制：
    Dubbo SPI 是对 Java 原生 SPI 的增强，核心是基于配置文件实现接口的动态扩展：
        ①定义接口并标注 @SPI；
        ②在 META-INF/dubbo/ 下配置接口实现类（key = 类全路径）；
        ③通过 ExtensionLoader 加载实现类，支持按需加载、自适应扩展、自动包装，是 Dubbo 插件化（如协议 / 序列化 / 负载均衡）的核心。
4.Dubbo 的核心架构 / 调用流程？
    ① 注册中心（Nacos/ZK）：服务注册与发现；
    ② 消费者：拉取服务列表，负载均衡选节点；
    ③ 远程调用：Netty 传输 + 序列化（Hessian/JSON）；
    ④ 监控 / 治理：统计调用、熔断降级、配置动态调整。
5.Dubbo 支持的负载均衡策略及场景？
    ① 随机（Random）：默认，适合无状态服务；
    ② 轮询（RoundRobin）：按节点顺序调用，适合节点性能一致；
    ③ 最少活跃调用（LeastActive）：选请求数最少的节点，适合性能不均场景；
    ④ 一致性哈希（ConsistentHash）：相同参数路由到同一节点，适合有状态服务。
6.Dubbo 如何解决服务调用超时 / 重试问题？
    ① 超时配置：设置timeout（接口 / 方法级别），避免长阻塞；
    ② 重试配置：设置retries，失败后重试其他节点（幂等接口才用）；
    ③ 兜底：结合降级策略，超时直接返回兜底数据，避免雪崩。
```

### CI/CD：Jenkins|Docker|Kubernetes

```java
Jenkins「自动化部署流程、构建排查、多环境部署」，主打 CI/CD 落地；
Docker「镜像 vs 容器、数据持久化、跨主机通信」，主打容器化基础；
K8s「Pod 生命周期、控制器区别、服务发现」，主打容器编排核心。
//Java 微服务部署流程：Jenkins 编译打包 → 构建 Docker 镜像 → 推镜像仓库 → K8s Deployment 部署

Jenkins
1. Jenkins 实现自动化部署的核心流程？
    ① 配置代码仓库（Git/SVN），触发方式选 WebHook（代码提交触发）；
    ② 构建步骤：拉取代码→编译打包（Maven/Gradle）→生成镜像 / 包；
    ③ 部署步骤：推送镜像 / 传输包→执行脚本部署到服务器；
    ④ 后置：构建通知（邮件 / 钉钉）、归档构建产物。
2. Jenkins 构建失败如何排查？
    ① 查构建日志（Console Output），定位编译 / 脚本 / 网络报错；
    ② 验证代码分支 / 版本是否正确，依赖包是否下载成功；
    ③ 检查服务器环境（JDK/Maven 版本）、权限（部署目录读写）；
    ④ 回滚到上一次成功构建版本，对比差异定位问题。
3. Jenkins 如何实现多环境（测试 / 生产）部署？
    ① 配置多套 Job，分别对应测试 / 生产环境；
    ② 用参数化构建（Parameterized Build），指定环境参数（如 env=prod）；
    ③ 生产环境加人工审核步骤，避免误部署；
    ④ 不同环境配置不同的服务器凭证、部署脚本。

Docker
1. Docker 镜像和容器的核心区别？
    ① 镜像：只读模板（包含代码 / 依赖 / 环境），静态不可变，是容器的基础；
    ② 容器：镜像的运行实例，可读写，独立隔离，启动即创建、停止即销毁；
    ③ 关系：一个镜像可创建多个容器，容器修改可提交为新镜像。
2. Docker 容器数据持久化方案？
    ① 数据卷（Volume）：最常用，宿主机目录映射到容器，独立于容器生命周期；
    ② 绑定挂载（Bind Mount）：直接挂载宿主机指定路径，灵活但耦合宿主机；
    ③ 临时文件系统（tmpfs）：数据存内存，适合临时数据，容器销毁即丢失。
3. Docker 如何解决容器跨主机网络通信？
    ① 用 Docker Compose（单机多容器）+ Docker Swarm（集群）；
    ② 第三方网络插件（如 Calico/Flannel）；
    ③ 核心：容器分配虚拟 IP，集群内通过 overlay 网络实现跨主机互通；
    ④ 实战：Java 服务部署时，通过容器名称 / 服务名解析通信。

Kubernetes（k8s）
1. K8s Pod 的核心特性及生命周期？
    ① 特性：最小部署单元，包含一个/多个容器，共享网络/存储，原子性调度；
    ② 生命周期：Pending（调度中） → Running（运行） → Succeeded/Failed（结束）；
    ③ 关键：Pod 销毁后不可恢复，通过控制器（Deployment）重建。
2. K8s Deployment 和 StatefulSet 的区别？
    ① Deployment：无状态服务（如 Java 微服务），Pod 无固定名称 / 存储，支持滚动更新；
    ② StatefulSet：有状态服务（如数据库 / 中间件），Pod 有固定名称 / 持久存储，有序部署 / 更新；
    //场景：微服务用 Deployment，MySQL/Redis 集群用 StatefulSet。
3. K8s 实现服务发现和负载均衡的方式？
    ① 服务发现：通过 Service 资源，给 Pod 集群分配固定虚拟 IP（ClusterIP），通过服务名解析；
    ② 负载均衡：Service 内置负载均衡，按策略（轮询 / 随机）转发请求到 Pod；
    ③ 外部访问：NodePort（节点端口）、LoadBalancer（云厂商负载均衡）、Ingress（七层路由）。
```
...
1.Netty如何实现高性能的吗？
    ...
    4)通信协议、序列化等其他角度的优化
2. Netty 高性能的核心原因？
    ① 线程模型：Reactor 主从多线程模型，Boss 线程处理连接、Worker 线程处理读写，避免阻塞；
    ② 零拷贝：通过 CompositeByteBuf/Direct Buffer 减少内存拷贝；
    ③ 异步非阻塞：基于 NIO + 事件驱动，配合池化（内存 / 线程），减少资源开销；④ 粘包拆包：内置解码器（LineBasedFrameDecoder），高效处理 TCP 粘包。
3. Netty 如何解决 TCP 粘包 / 拆包问题？
    ① 固定长度解码器（FixedLengthFrameDecoder）：按固定字节拆分；
    ② 换行符解码器（LineBasedFrameDecoder）：按 \n/\r\n 拆分；
    ③ 自定义分隔符 / 长度域解码器（LengthFieldBasedFrameDecoder）：最常用，按消息头长度字段拆分；
    ④ 核心：解码器在读取数据时，先拼接完整消息再交给业务处理。
4. Netty 的核心组件及作用？
    ① Bootstrap/ServerBootstrap：客户端 / 服务端启动器，配置线程模型、端口等；
    ② Channel：通信通道，封装 Socket 连接，管理读写状态；
    ③ ChannelHandler：业务处理核心（入站 / 出站处理器），如解码、业务逻辑；
    ④ EventLoop：线程 + 任务队列，负责处理 Channel 的所有事件，单线程串行执行避免并发问题。

WebSocket
1. WebSocket 与 HTTP 的核心区别？
    ① 通信模式：HTTP 是单向请求 - 响应（客户端主动），WebSocket 是双向全双工（前后端互发）；
    ② 连接状态：HTTP 无状态、短连接，WebSocket 是长连接、一次握手持续通信；
    ③ 协议标识：HTTP 用 http/https，WebSocket 用 ws/wss，握手时依赖 HTTP 升级请求。
2. WebSocket 握手流程及核心字段？
    ① 客户端发 HTTP 请求，携带Upgrade: websocket+Sec-WebSocket-Key；
    ② 服务端验证 Key，返回101 Switching Protocols+Sec-WebSocket-Accept；
    ③ 握手成功后，切换为 WebSocket 协议，开始双向通信。
3. WebSocket 如何解决连接断开 / 重连问题？
    ① 心跳机制：前后端定时发送 ping/pong 帧，检测连接状态；
    ② 前端监听onclose/onerror事件，触发自动重连（加退避策略，避免频繁重试）；
    ③ 服务端维护连接池，清理超时连接，重连后恢复会话（如通过 token 绑定用户）。
```
...
CAP理论: 
    ...
    即一致性（C：Consistency）,可用性（A：Availability）,分区容错性（P：Partition tolerance）
      C【一致性】在某个节点上执行变更操作后，用户可以立即从其他任意节点上读取到变更后的数据。
      A【可用性】对于用户的每一个操作请求总是能够在有限的时间内返回结果，有限时间内 + 明确响应结果。
      P【分区容错性】分布式系统在遇到网络分区时，仍需要能够对外提供一致性和可用性的服务，除非是整个网络环境都发生了故障。
    分布式系统中网络分区是不可避免的，不可能永远不出现网络故障，所以分区容错性 P 必须要保证，所以通常为AP/CP
      AC - '单节点'避免P分区容错性问题，即单机部署运行，如单机版的MySQL/Redis。
      AP - 如果发生了网络分区故障，节点间的数据就无法同步。因此在'故障修复前都需要放弃对外提供服务'，直至网络恢复，数据到达一致为止。如Nacos默认高可用
      CP - '放弃数据的强一致性，但保证数据的最终一致性'。不论是什么系统，数据最终都需要保持一致，否则整个系统就无法使用。短暂时间窗口会存在数据不一致。
BASE理论:
    即基本可用（Basically Available）、软状态（ Soft State）、最终一致性（ Eventually Consistent）
    它是对 CAP 理论中 AP 策略的延伸。//高可用+最终一致性
    核心是即便无法做到强一致性，但每个系统都应该根据自身业务特点，采取适当的方式来保证系统的'最终一致性'。
```
...
1. DDD:
    聚焦业务领域的设计思想，核心是
    1. “以业务为中心” 替代 “以技术为中心”，通过拆分领域模型（限界上下文、聚合根、值对象等），将复杂业务拆分为高内聚、低耦合的领域模块
    2. 解决传统分层架构 “业务与技术脱节、代码臃肿” 的问题
    3. 落地时先梳理领域模型，再映射为代码（领域层为核心），适配微服务拆分
    //我在 SaaS 项目中用 DDD 拆分了课程、订单、支付领域，提升了代码复用性和业务扩展性，维护起来代码也清晰明了，效率和质量都有所提高。
    传统 MVC 三层是按技术职责分层：Controller、Service、Dao，核心是数据流转，业务逻辑散在 Service 里，容易变成事务脚本、大泥坑，后期维护和扩展成本很高。
    然而 DDD 是按业务领域划分，强调领域对象、聚合根、领域服务、限界上下文，核心是业务语义，把业务规则内聚在领域层，代码更贴近业务，可读性、可复用性、可扩展性更强。
    简单说：三层是面向数据和技术，DDD 是面向业务和模型。
    //复杂业务、中大型项目、长期迭代的系统用 DDD 优势非常明显；三层架构 MVC 更适合简单、小型、快速上线的项目。
2. 举个最简单的 DDD 例子：电商「下单」场景
    传统三层写法（MVC）
        Controller 接收参数
        Service 里一大段代码：
        校验库存 → 扣库存 → 计算价格 → 创建订单 → 扣优惠券 → 记录日志
    	//所有逻辑堆在一个 createOrder() 方法里，几百行，越维护越烂。
    DDD 写法（按业务模型拆分），直接划分领域对象：
        Order 订单（聚合根）
        Product 商品
        Coupon 优惠券
        Money 金额值对象
    然后把业务规则放在领域类里面：
        Order.canCreate() //判断能不能下单
        Product.deductStock() //扣库存（带规则）
        Coupon.use() //使用优惠券（带过期、是否可用规则）
        Money.calculate() //计算价格
    Service 只做编排，不写业务逻辑：
        order = Order.create(product, coupon, address);
        orderRepository.save(order);
    一句话总结：
    传统三层是把逻辑写在 Service 里，DDD 是把业务规则封装到领域模型里。
    比如下单，三层是一堆过程代码，DDD 是让「订单、商品、优惠券」这些业务对象自己管自己的规则，代码更清晰、更好维护、更贴合真实业务。
```
...
2.小张看最近的大米价格，他打开百度，进行了一番操作，最后获得最近的大米价格。
	...
	http → 接口 → ip → 机器 → 端口 → 进程 → 服务 → 线程 → 代码逻辑 → 数据库 → 原路返回
...
```java
1.你在项目中怎么使用 AI 编程工具？
    我主要用 AI 做代码生成、Bug 定位、接口文档生成、单元测试编写、复杂逻辑梳理。
    比如用 AI 快速生成 CRUD、工具类、SQL 优化、Redis 分布式锁、Lua 脚本等重复代码；
    遇到线上问题时，用 AI 分析异常栈、定位性能瓶颈；
    还会用 AI 优化 Prompt，把复杂需求拆解成可执行代码，提升开发效率 30% 以上，同时保证代码质量和规范。
2.AI 写的代码有什么风险？怎么避免？
    风险主要有逻辑错误、安全漏洞、冗余代码、不符合业务场景、依赖不兼容。
    我会做三层校验：
        第一，理解代码逻辑，不盲目复制；
        第二，代码审查 + 单元测试，覆盖边界条件；
        第三，用安全扫描工具检查漏洞，比如 SQL 注入、未关闭连接、权限问题。
    AI 是辅助，我负责架构、业务正确性和最终验收。
3.你怎么写高质量 Prompt 让 AI 写代码？
    我遵循清晰目标 + 上下文 + 约束 + 示例四步写法。
        先说明场景、技术栈、输入输出；
        再给业务规则、字段定义、异常处理；
        最后指定代码风格、注释、函数结构。
    比如让 AI 写接口，我会明确请求参数、返回结构、权限校验、日志、异常捕获，这样 AI 输出的代码可直接使用，不用大量修改。
4.AI 会不会替代程序员？你的看法
    AI 不会替代程序员，但会用 AI 的程序员会替代不用 AI 的。
    AI 擅长重复编码、查资料、写模板；而程序员核心价值是需求分析、架构设计、业务理解、问题排查、系统调优、方案决策。
    我把 AI 当成高效助手，专注更有价值的架构与复杂问题，提升整体交付能力。
5.你用 AI 做过哪些实际项目优化？
    我用 AI 做过接口性能优化、SQL 索引优化、JVM 参数调优、代码重构、定时任务逻辑梳理。
    比如让 AI 分析慢查询语句，给出索引方案和执行计划解释；
    用 AI 生成高并发下的幂等、限流、降级代码；
    还让 AI 画架构图、时序图、梳理业务流程，大幅减少文档与调试时间，保证系统稳定。
6.AI 生成代码有版权风险吗？项目中怎么处理？
    有风险，可能存在开源协议冲突、代码抄袭、隐私泄露。
    我在公司项目中遵守三点：
        第一，不直接上传敏感业务代码、配置、密钥给公共 AI；
        第二，只让 AI 生成通用逻辑、工具类、框架模板；
        第三，对生成代码必须审查、重构、单元测试，确保安全合规，不直接照搬。
7.你用过哪些 AI 编程工具？区别是什么？
    我常用 ChatGPT、DeepSeek、通义千问、IDE 内置 AI（如 Copilot）、字节的Trae。
    公共大模型适合复杂问题、架构思路、调优方案；
    IDE 内置 AI 适合实时补全、单函数生成、错误提示。
    我会根据场景选择：架构与思路用大模型，编码实时辅助用 IDE 插件，效率最高。
8.怎么让 AI 帮你快速定位线上 OOM、死锁、慢接口？
    我会把异常日志、GC 日志、线程栈、接口耗时发给 AI，让它分析原因。
    比如 OOM 时，让 AI 判断是内存泄漏、堆设置不合理还是大对象问题；
    死锁时让 AI 分析锁顺序与竞争点；
    慢查询让 AI 看执行计划与索引。
    AI 能快速给出方向，我再用 Arthas、MAT 工具验证，定位效率提升很多。
9.你认为 AI 对后端开发最大的价值是什么？
    最大价值是把程序员从重复劳动中解放出来。
    以前写大量模板代码、配置文件、工具类、文档、测试用例，现在 AI 几分钟完成。
    我们可以专注架构设计、高并发、高可用、业务抽象、领域建模，比如我现在用 AI 加速 DDD 拆解、微服务划分、流程设计，整体研发效率明显提升。
10.未来你会怎么持续使用 AI 提升竞争力？
    我会从三个方向提升：
        第一，深度使用 AI 做架构设计与方案对比；
        第二，用 AI 学习新技术、源码、中间件原理，快速补齐知识；
        第三，把 AI 能力融入团队，形成需求→AI 生成→审查→测试→部署的高效流程。保持技术敏感度，让 AI 成为自己的核心竞争力。
```
