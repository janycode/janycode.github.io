...
8. 登陆鉴权如何做的？
    我从认证、鉴权、安全三层做完整登录体系：
    - 认证流程：前端提交'账号密码'，后端用BCrypt'不可逆加密'方式比对库中密文，验证通过后，'签发'JWT短时效AccessToken+长时效RefreshToken双令牌；
    - 前端维护：封装'请求拦截器'无需手动加Auth头，由Cookie自动透传，'响应拦截器'自动处理401、无感刷新令牌；刷新Token需通过后端接口完成（如/auth/refresh）；
    - 后端鉴权：Token存在HttpOnly'Cookie'防XSS（后端），通过'网关/拦截器'统一校验 Token 合法性与过期时间，解析用户信息；
