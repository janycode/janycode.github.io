...
8. 登陆鉴权如何做的？
    我从认证、鉴权、安全三层做完整登录体系：
    - 认证流程：前端提交'账号密码'，后端用BCrypt'不可逆加密'方式比对库中密文，验证通过后，'签发'JWT 短时效 AccessToken + 长时效 RefreshToken 双令牌；
    - 前端维护：Token 存在 HttpOnly 'Cookie' 防 XSS，封装'请求拦截器'统一带鉴权头，'响应拦截器'自动处理 401、无感刷新令牌；
    - 后端鉴权：通过'网关/拦截器'统一校验 Token 合法性与过期时间，解析用户信息；基于 RBAC 模型 做角色→菜单→按钮三级细粒度'权限控制'；
