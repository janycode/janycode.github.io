...
1. display:none; 与 visibility:hidden; 的区别
    相同：都可以让元素不可见
    区别：
        1) display: none 是「物理移除」，不占空间、无交互； visibility: hidden 是「视觉隐藏」，占空间、可交互；
        2) 动画/保留空间场景用 visibility，彻底移除场景用 display；
        3) 性能层面，visibility 仅触发 重绘(★)性能更优；display 触发页面重排+重绘。
2. 外边距折叠
    ...
    4) 动态rem方案(★)
      rem 和媒体查询配合实现响应式布局。
    	...
    	- rem：相对于【根元素的 font-size】的大小，如
    	    css 中基准 fontSize设置，如: html{ font-size: 100px; }
    	    js  中动态 fontSize设置，如: iphone6的750px为标准宽度等比缩放设计稿，因为设备缩放比是2，所以使用375px进行计算，计算方式:
...
5. Vue响应式数据原理？
    响应式原理-vue2（如果是 Vue3 则是通过 Proxy 代理对象的方式拦截，拦截面更广）：
    ...
    Object.defineProperty-vue2 与 Proxy-vue3 优劣对比？
      - Object.defineProperty: 遍历对象属性直接修改，兼容性好，支持IE9，无法监听数组改变、class改变、Map、Set结构改变。
...
29. Vue 生命周期中 create 和 mount 的区别:
    create: 组件初始化阶段，主要完成数据观测data observer、属性和方法的运算、watch/event事件回调，还未生成真实DOM，无法获取DOM元素
...
2. Vue3响应式失效排查：解决数据更新但视图不刷新
    ...
    经典实例: 数组的 filter() / concat() / slice()等这类返回新数组即不影响原数组的方法，均脱离了Proxy代理
...
8. 登陆鉴权如何做的？
    我从认证、鉴权、安全三层做完整登录体系：
    - 认证流程：前端提交'账号密码'，后端BCrypt'不可逆加密'比对库中密文，验证通过则'签发'JWT短时效AccessToken+长时效RefreshToken双令牌；
    - 前端维护：封装'请求拦截器'无需手动加Auth头，由Cookie自动透传，'响应拦截器'自动处理401、无感刷新令牌；刷新Token请求接口如/auth/refresh；
    - 后端鉴权：通过响应头Set-Cookie将Token存在'Cookie'设HttpOnly防XSS，通过'网关/拦截器'统一校验 Token 合法性与过期时间，解析用户信息；
    - 安全兜底：密码不可逆加密、接口'限流'防暴力破解，登出/强制下线时将 'Token 加入黑名单失效'，杜绝越权访问。
    RefreshToken时效和机制设计:
    -- 令牌设计：AccessToken（2小时，短时效）用于接口鉴权，RefreshToken（7天，长时效）仅用于刷新令牌，存储在后端 Redis 并绑定用户ID/设备信息；
    -- 触发时机：前端请求拦截器捕获 401（Token 过期），自动携带 RefreshToken 调用/auth/refresh接口；
    -- 后端校验：校验RefreshToken合法性（是否存在Redis、未过期、设备匹配），验证通过后签发新AccessToken（旧的立即失效）；
    -- 安全兜底：RefreshToken 仅允许使用 1 次（刷新后旧的立即失效），若校验失败直接返回 403，前端引导重新登录；前端封装无感刷新逻辑，避免用户感知。

9. 无 Sentinel 时，优先用分层限流（单机场景）：Nginx（网关层配置limit_req_zone）→ Redis+Lua（集群）→ Guava（单机）
   集群场景，使用 Sentinel 是最佳方案了。

10. 秒杀的前后端全栈全链路设计
    前端: 流量拦截、初步防刷、体验
    1. 秒杀前准备（防重复 / 防刷）
     - 【秒杀资格校验】：接口获取资格校验、'资格缓存'到localStorage；
     - 【按钮防重复点击】：用防抖（'debounce'）限制按钮点击频率（比如 500ms 内只能点 1 次），配合 loading 状态，防止用户重复提交请求；
     - 【秒杀时间同步】：前端'从后端获取服务器时间'（避免客户端时间不准），秒杀未开始时按钮置灰，且做 “秒杀倒计时”，倒计时结束才开放请求。
    2. 秒杀请求发送（降峰 + 兜底）
     - 【请求限流】：前端对秒杀接口做请求次数限制（比如'单用户 1 分钟内最多发 3 次请求'），拦截恶意刷请求的用户；
     - 【请求降级兜底】：若请求超时/失败，返回 “当前秒杀人数过多，请稍后重试” '友好提示'；
     - 【静态资源优化】：秒杀页面所有静态资源（图片、CSS、JS）走 'CDN'，且做懒加载 / 预加载，避免页面加载慢导致用户错过秒杀。
    3. 秒杀结果展示（实时 + 兜底）
     - 【轮询获取结果】：秒杀请求不等同步返回（避免接口超时），而是返回 “秒杀请求已提交”，然后前端通过'轮询（1s/次）'获取最终结果；
    后端: 高可用、防超卖
    1. '接入层'：Nginx 限流（第二道流量拦截线）
     - 【IP 限流】：Nginx 配置limit_req_zone，'限制单 IP 每秒请求数'（比如 10req/s），直接拦截大部分恶意刷请求；
     - 【静态资源缓存】：秒杀页面的 HTML/CSS/JS 在 'Nginx 层缓存'，不穿透到应用服务器；
     - 【请求转发】：将秒杀请求转发到专门的秒杀集群（和普通业务'集群隔离'），避免秒杀拖垮核心业务。
    2. '应用层'：流量控制 + 核心逻辑（第三道拦截线）
    （1）流量控制（Sentinel/Guava RateLimiter）
     - 【接口限流】：用 Sentinel 对秒杀接口做集群'限流'（比如集群总QPS限制3000）+ 单机限流（比如限制QPS800），超阈值返回“秒杀火爆，请稍后重试”；
     - 【服务熔断降级】：若秒杀接口的下游服务（如库存服务）异常，触发 Sentinel 熔断，'熔断时返回降级结果'（而非等待超时），避免服务雪崩。
    （2）核心业务逻辑（防超卖是核心）
     - 【Redis预减库存】：用'decr'原子操作（非脚本），避免并发下库存计算错误，这是第一道库存校验；
     - 【分布式锁】：用 Redis 的'SETNX+过期时间'（或'Redisson'）实现，锁定 “活动ID + 商品ID”，防止同一商品最后 1 件库存被多用户抢占；
     - 【MQ 削峰】：秒杀请求不直接操作数据库，而是发送到'MQ'，消费端异步处理，把瞬间高并发转换成串行处理，避免数据库压垮；
     - 【数据库乐观锁】：最终扣库存时，用WHERE 商品ID=? AND 库存>0（或版本号）做'乐观锁'，这是最后一道防超卖防线；
     - 【库存回补】：若 MQ 消费失败 / 库存不足，必须把 Redis 中预减的'库存回补'，避免 Redis 库存和数据库库存不一致。
    3. '缓存层'：多级缓存兜底
     - 【热点商品缓存】：秒杀商品的基本信息（名称、价格、库存）缓存到 Redis + 本地缓存（Caffeine），'数据库挂了也能返回基础信息'；
     - 【秒杀结果缓存】：用户秒杀结果缓存到 Redis，前端'轮询时直接查 Redis'，不穿透到数据库。
    4. '兜底'降级策略
     - 【非核心流程降级】：秒杀成功后的短信通知、用户积分增加等，若服务异常则降级，'优先保证核心流程' “扣库存 + 生成订单” ；
     - 【库存兜底】：若 Redis 宕机，直接返回 “秒杀暂未开启”，'避免请求全部打到数据库'；
     - 【接口幂等性】：用'用户ID+商品ID+活动ID'作为唯一幂等号放redis缓存直到活动结束，避免用户重复下单（比如网络延迟导致重复提交）。
```
...
2) 关键词高亮问题（原理和字符串敏感词替换一样，但是用在关键词高亮上属于亮点）
