...
spring七大模块、springboot启动过程、自动装载、约定大约配置逻辑

redis过期策略、删除策略、雪崩、穿透、击穿、解决方案、redisson、分布式锁、lua、布隆过滤器

mysql索引底层数据结构、B树、B+树、分布式事务常用方案、分库分表使用场景

hashmap1.7与1.8不同、concurrenthashmap底层不同、1.8的底层

cas+syncronized、引申出1.8为什么用 syncronized、syncronized的锁升级过程、cas原理
...
lock锁的底层AQS、信号量、原子类、volatile、JMM内存模型、单例DCL可见性、禁止指令重排序等

项目中用到了什么、有什么问题、怎么解决，讲明白一个项目或模块

线程池、rocketMQ底层刷盘和复制逻辑、怎么保证消息不丢失和重复消费、顺序消费问题，cap和base理论等



算法：求和树的路径

算法：多线程，模拟实现并发调用100个微服务的接口，控制总体的超时时间
