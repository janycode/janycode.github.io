...
8.JDK 1.8新特性：Lambda表达式、Stream API、函数是接口、接口default方法、接口static方法
...
14.int和Integer有什么区别?谈谈Integer的值缓存范围。
    ...
    缓存值范围 -128~127存储在一个static final常量cache中 JDK1.5后提供，
    赋值时调valueOf()方法会重用这些缓存的常用数，显著提高性能。
15.接口和抽象类的区别？
...
2.死锁条件与如何避免：资源分配不当/线程推进顺序不当，避免：打破 互斥/不可抢占/占有且申请/循环等待
...
13.信号量Semaphore：
	...
	一个计数器，一个等待队列，三个方法（init，down，up），Semaphore是用来保护一个或者多个共享资源的访问：
...
2.JVM内存模型：PC/堆/栈/元空间，1.8-元数据区方法区变为本地内存元空间
3.类加载过程？
    类加载三个步骤：加载、链接、初始化。
    加载：将字节码加载到JVM中。
    链接：将类信息链接到JVM中运行。
...
12.指令重排？为什么需要指令重排序？
	只要程序的最终结果与顺序执行的结果保持一致，则虚拟机就可以进行排序，此过程就叫指令重排序。
...
23.分库分表后如何分页：
	1)全局查询法：同时查询多个表，再在内存中排序，取出对应页码的数据（缺点：性能低，CPU和内存占用高）
...
6.分布式事务常用方案：
	...
	TCC：柔性事务，大名鼎鼎的补偿事务，是互联网环境最常用的分布式事务。
          核心思想是：为每一个操作，都准备一个确认动作和相应的补偿动作，一共3个方法: try(),confirm(),cancel()
	SAGA：柔性事务，主要处理的是长活事务，但它不保证ACID，只保证最终一致性。靠消息驱动完成事务运转。
	本地消息表：将需要分布式处理的任务通过消息日志的方式来异步执行。要靠MQ去实现，有局限性。
	可靠消息最终一致性：去掉了本地消息表，轮询 prepare 消息状态来重试或者回滚该消息替代。
	最大努力通知：一种衰减式的补偿机制，最清晰的例子就是如支付回调。
7.Seata分布式事务框架:
...
1.RESP协议：
	...
	实现简单、解析快、人类可读的特点，底层是TCP连接方式，通过tcp进行数据传输，根据解析规则解析相应信息完成交互。
2.Redis淘汰策略：
...
10.String底层实现为什么是SDS，SDS和C字符串的比较？
    * SDS simple dynamic string
      ...
      * buf：字节数组，用于保存字符串。
    原因：1、获取字符串长度的复杂度降低；2、避免内存溢出；3、空间预分配；4、惰性删除
...
15.Redis IO多路复用：一个服务端进程可以同时处理多个套接字描述符。
	IO 多路复用只需要一个进程就能够处理多个套接字，从而解决了上下文切换的问题。主要方法 select()/poll()/epoll()
...
9.AOP的应用：
    ...
    Synchronization 同步
10.@Autowired的实现原理：
	默认使用byType来自动装配（@Resource<按名称注入> = @Autowired<按类型注入> + @Qualifier<按名称注入>）。
...
1.nacos注册中心实现原理：
    服务注册 + 服务发现
    1)业主服务启动通过 openAPI 发起服务注册
    2)建立心跳机制（10s）检查服务状态[检测到服务异常则通过UDP协议推送更新]
    3)服务注册表中维护服务实例名称、ip地址端口、健康状态等信息，以及控制上线/下线
    4)调用方发起调用时，通过注册中心的服务实例名称，找到ip地址端口发送请求和接收响应
...
3.MyBatis实现原理：
	...
	SqlSession包含了执行sql所需要的所有方法，可以通过【SqlSession】实例直接运行映射的sql语句，
...
1.RocketMQ底层刷盘逻辑:
	同步刷盘、异步刷盘
    ①同步刷盘：Producer发送消息 → Broker接收 → PageCache内存写入 → 刷盘线程 → 当前线程等待 →
...
3.RocketMQ怎么解决消息丢失和重复消费、顺序消费问题:
	...
	顺序消费：普通>同一个消费队列是有顺序的，重启不保证顺序；严格>同一个Topic严格按照FIFO顺序消费
             ①通过类似于订单id的hash选择相同队列，单线程消费
			②多线程消费时，消费者内部建立内存队列，消息拉取到内存队列后，分发给不同线程，进行顺序消费
4.
```
